...
function License(): JSX.Element {
  document.addEventListener('keydown', function (event) {
    if (event.key === 'Escape' || event.key === 'Esc' || event.keyCode === 27) {
      window.electron.ipcRenderer.send('window:closeLicense')
    }
  })
  ...
  return (
    <main className={`w-full h-full relative`}>
      ...
      <img src={licenseBackground} alt="" className={`absolute bottom-0 right-0`} />
      <span className={`w-full h-full absolute top-0 left-0 bg-stone-300 bg-opacity-80 -z-0`}>
        <div
          className={`w-full h-full flex-grow flex px-5 mt-7 mb-10 text-black flex-col overflow-y-auto`}
        >
          <Main />
...
const Main = () => {
  return (
    <>
      <div className={`licence font-mono`} dangerouslySetInnerHTML={{ __html: html }}></div>
    </>
  )
}

const html = `<div style="text-align:justify;  padding: .5em;">
